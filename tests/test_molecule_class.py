...
def test_ecm_onpath_cartesian(tmp_path, capfd):
    ...
    method = {'fourcomp': False, 'debug' : 1, 'X2C': True}
    zrate, ECMs_NR, ECMs_x2c, ECMs_4c = c.ECM_on_path(name=path+'CFMAR_chiral', method_dict=method, gto_dict=GTO, lim_inf=minimo, lim_sup=maximo, points=puntos, cartesian=False)
...
def test_ecm_onpath_4c(tmp_path, capfd):
    ...
    zrate, ECMs_NR, ECMs_x2c, ECMs_4c = c.ECM_on_path(name=path+'CFMAR_chiral', method_dict=method, gto_dict=GTO, lim_inf=minimo, lim_sup=maximo, points=puntos)
