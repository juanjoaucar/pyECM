...
def test_create_mol_files(tmp_path):
    ...
    text_xyz_050 = "16   \nXYZ file   \nN -5.3367861987535514e-06 -2.8660119784085225e-06 -0.2912757784571628\nC 0.17875410295182356 1.0901510921606696 0.19089078254252342\nC 1.7018206179625015 1.3238830959255414 0.23967371236763352\nC 2.265610222676117 -0.10894431880701205 0.16921303378075853\nC 1.0953649015743183 -0.9391752269864793 -0.13985706123930536\nN 2.1288927712001655 2.1419427818720536 -0.33459853351362945\nH 0.1570652971982 0.3849830429626081 -0.7545334259738417\nH -0.2111448312648552 0.774970594238776 0.6785225069581948\nH -0.37320488234197635 1.9806551309842066 0.035577191787877216\nH 1.978937701216539 1.7689099268611583 0.7227690177981916\nH 2.5603405573594484 -0.5075494885888936 0.6561992176301077\nH 3.153187715485455 -0.11447739383193592 -0.1495925457810876\nH 1.3857016170021408 -1.481413846764631 -0.5905440194751934\nH 0.736826194174581 -1.6779419380690448 0.22213236377625178\nH 3.1411187824257727 2.181181032447245 -0.3663605001171667\nH 1.7975436123221973 3.097659519280735 -0.28833176750451517\n"
...
def test_ecm_onpath_4c(tmp_path,capfd):
    ...
    zrate, ECMs_NR, ECMs_molcontr, ECMs_4c = c.ECM_on_path(name=path+'CFMAR_chiral', lim_inf=minimo, lim_sup=maximo, points=puntos, tracking=True, fourcomp=True, debug=1, basis_set='sto-3g')
...
def test_ecm_onpath_cartesian(tmp_path,capfd):
    ...
    zrate, ECMs_NR, ECMs_molcontr, ECMs_4c = c.ECM_on_path(name=path+'CFMAR_chiral', lim_inf=minimo, lim_sup=maximo, points=puntos, tracking=True, fourcomp=False, debug=1, basis_set='sto-3g', cartesian=True)
    out, err = capfd.readouterr() # For possible test on stdout
